...
def SEA_Templates():
    ...
    SEA_Samp  = pd.DataFrame(columns=['sample_id','group_id','organism_id', 'collection', 'collection_time', 'collection_time_unit', 'collection_time_unit_id', 'T0_definition'
                                      ...
                                      'replicates' ])
    SEA_Grop  = pd.DataFrame(columns=['group_id','experiment_id','group_id','group_size','reference_id','reference_source','comments'])
...
def SEA_Export(export, excep):
    try:
        for i in export:
            zeq = "ImmPort" + i.columns[0][0:-2]+".csv"
            i.to_csv(zeq)
...
#Sample Generation
SAMPLES = tables[9]
SAMPLES['sample_id'] = df_bios['EXPSAMPLE_ACCESSION']
...
SAMPLES['expsample_refence_name'] = 'GEO'
#df_geo = None
#df_bios2 = None
...
EXPER['study_id'] =  pd.concat([df_vis['STUDY_ACCESSION']])

#list_1d = ['Visit'] * df_vis.shape[0]
#list_2d = ['Experiment'] * df_exp.shape[0]                  
EXPER['experiment_type'] = 'experiment'

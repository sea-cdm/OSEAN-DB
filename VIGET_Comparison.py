...
test_g = GROUP[GROUP['experiment_id'].isin(jason['experiment_id'])]
test_g['groups_id'] = test_g['reference_id'].str[:5]
for col in ['reference_id']:
    test_g[col] = test_g[col].str.replace(" ", "")

test_g = test_g[['reference_id', 'max_age', 'min_age']]
test_g = test_g.rename(columns={'reference_id':'group_id'})
exportg = export.merge(test_g, on='group_id')
exportg.to_csv("Intermediate_New_E2F2.csv", sep = ',')
...
for pairs in range(0, exported.shape[0], 2):
    try:
        ...
        exported['gene_ratios'][pairs] = 2**(sample1 - sample2 )
        exported['gene_ratios'][pairs+1] = 2**(sample1 - sample2 )
...
happy_Oliver = load_VIGET(VIGET)

OLIVERS = happy_Oliver[happy_Oliver['gsm'].isin(test_filtered)]

VIGET_GSMS = GEXP[OLIVERS['gsm']]
VIGET_GSMS['gene_name'] = GEXP['GeneGene']
VIGET_E2FR = VIGET_GSMS.set_index('gene_name').T['E2F2']
...
VIG.rename(columns={'E2F2': 'gene_expression'}, inplace=True)
VIG['gene_name'] = 'E2F2'

VIG_counts = VIG[(VIG['immport_vaccination_time'] == time1) | (VIG['immport_vaccination_time'] == time2)]
counts = VIG_counts['immport_subject_accession'].value_counts()
VIG_counts2 = VIG[VIG_counts['immport_subject_accession'].isin(counts[counts >= 2].index)]
VIG_counts2.sort_values(['immport_subject_accession'])  

for pairs in range(0, VIG.shape[0], 2):
    try:
        sample1 = VIG.iloc[pairs]['gene_expression']
        sample2 = VIG.iloc[pairs+1]['gene_expression']
        print(2**(sample1 - sample2 ))
        VIG['gene_ratios'][pairs] = 2**(sample1 - sample2 )
        VIG['gene_ratios'][pairs+1] = 2**(sample1 - sample2 )
    except:
        0

VIG_counts2.to_csv('VIGET_raw_Gene_List_E2F2.csv')


# CODE FOR DUMMEIS AND EXAMPLES NOT TO BE USED

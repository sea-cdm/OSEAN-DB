...
if len(sys.argv) > 1:
   ...
   print("Please provide the relative path of the ImmPort metadata file as a command-line argument.")
...
#"C:\Users\huffmaar\Documents_Local\ALLSTUDIES-DR56_Metadata\workspace.txt"
...
wrk = pathway+'/workspace.txt'
epr = pathway+'/expsample_public_repository.txt'
df_wrk = pd.read_csv(wrk, sep = '\t')
...
df_ir = df_ir[df_ir['EXPOSURE_PROCESS_REPORTED'] == 'vaccination'] #HUH... ODD
...
STUDY['study_id'] = df_sdy['WORKSPACE_ID']
STUDY['reference_id'] = df_sdy['STUDY_ACCESSION']
STUDY['study_description'] = df_sdy['BRIEF_DESCRIPTION']
tables[0] = STUDY

EXPER = tables[2]
EXPER['experiment_id'] = pd.concat([df_vis['PLANNED_VISIT_ACCESSION'], df_exp['EXPERIMENT_ACCESSION']])
EXPER['study_id'] =  pd.concat([df_vis['WORKSPACE_ID'], df_exp['WORKSPACE_ID']])
...
df_bios = df_bios.merge(df_epr, on='EXPSAMPLE_ACCESSION')
SAMPLES = tables[9]
SAMPLES['biosample_reference_id'] = df_bios['BIOSAMPLE_ACCESSION']
SAMPLES['biosample_type'] = df_bios['TYPE']
SAMPLES['expsample_reference_id'] = df_bios['REPOSITORY_ACCESSION'] #Use EXPSAMPLE_ACCESSION IF TBD
SAMPLES['expsample_reference_source'] = df_bios['REPOSITORY_NAME']
SAMPLES['expsample_type'] = df_bios['SUBTYPE']
...
SAMPLES['organism_id'] = df_bios['SUBJECT_ACCESSION']
SAMPLES['biosample_reference_source'] = 'ImmPort'
